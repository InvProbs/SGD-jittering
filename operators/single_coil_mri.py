"""
Code adopted from the paper "Deep Equilibrium Architectures for Inverse Problems in Imaging"
https://github.com/dgilton/deep_equilibrium_inverse/blob/main/operators/singlecoil_mri.py
"""
...
class cartesianSingleCoilMRI(LinearOperator):
    def __init__(
        ...
        kspace_mask = create_mask(
            shape=[img_dim, img_dim, 2],
            acceleration=mri_acc,
    ...
    def forward(self, input):
        ...
        complex_input = torch.view_as_complex(input.permute((0, 2, 3, 1)).contiguous())
        complex_input = torch.fft.ifftshift(complex_input, (1, 2))
        kspace = torch.fft.fftn(complex_input, dim=(1, 2), norm='ortho')

        if self.mask is not None:
            # shift freq to center
            shifted_kspace = torch.fft.fftshift(kspace, (1, 2))
            masked_kspace = shifted_kspace * self.mask + 0.0
            masked_kspace = torch.fft.ifftshift(masked_kspace, (1, 2))

        if remove_batch:
            masked_kspace = masked_kspace[0]

        return torch.view_as_real(masked_kspace)

    def gramian(self, input):
        return self.adjoint(self.forward(input))
        #         if input.ndim == 4:
    ...
    def adjoint(self, input):
        ...
        complex_input = torch.view_as_complex(input)
        realspace = torch.fft.ifftn(complex_input, dim=(1, 2), norm="ortho")
        realspace = torch.fft.fftshift(realspace, dim=(1, 2))
        realspace = torch.view_as_real(realspace).permute((0, 3, 1, 2)).contiguous()

        if remove_batch:
            realspace = realspace[0]
        return realspace
