...
parser.add_argument("--data_path", type=str, default='D:/data/Seis/')
...
net = nets.seis_gd(forward_op, dncnn, args).to(args.device) # SGD jitter noise added in net
    ...
    with tqdm(total=(ts_length - ts_length % args.batch_size_val)) as _tqdm:
        ...
        for X, y in val_loader:
            ...
            if args.test_mode == 'AT':
                delta, loss_list = tr.PGD(net, X, y, y, args.epsilon, args.PGD_stepsize, args.n_PGD_steps, dim=3, eps_mode='l2')
                y += delta
